...
class FieldDraggingTool extends go.DraggingTool {
  ...
  doMouseUp() {
    ...
    diagram.remove(this.temporaryPart);
    console.log(dest && dest.data);
    if (dest && dest.data.group === "target") {
      const from = data.key;
      const to = dest.data.key;
      const newLinkData = { key: "123213", from, to, category: "Mapping" };
      diagram.model.addLinkData(newLinkData);
      const { key, group, name } = data;
      diagram.model.addNodeData({ key, group, name });
    } // end
